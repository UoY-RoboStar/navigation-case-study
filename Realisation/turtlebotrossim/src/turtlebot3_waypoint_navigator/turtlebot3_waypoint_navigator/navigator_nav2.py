...
class TurtleBot3WaypointNavigatorNav2(Node):
    ...
    def set_initial_pose(self, x: float = -2.0, y: float = -0.5, theta: float = 0.0) -> None:
        ...
        # CRITICAL FIX: Wait for publisher to establish connection with AMCL
        ...
        while self.initial_pose_pub.get_subscription_count() < 1 and wait_count < 50:
            ...
            if wait_count % 10 == 0:
                self.get_logger().info(
                    f'Still waiting for AMCL subscriber... ({wait_count/10:.0f}s elapsed)'
                )

        if self.initial_pose_pub.get_subscription_count() < 1:
            self.get_logger().warn(
                'No subscribers to /initialpose after 5 seconds. '
                'AMCL may not be running or not ready yet.'
            )
        else:
            self.get_logger().info(
                f'AMCL subscriber connected! '
                f'({self.initial_pose_pub.get_subscription_count()} subscriber(s))'
            )

        # Publish the initial pose multiple times to ensure delivery
        ...
        for i in range(10):
            ...
            if i == 0:
                self.get_logger().info('First initial pose message sent')
            time.sleep(0.2)  # 200ms between publications

        self.get_logger().info('Initial pose published to AMCL (10 times over 2 seconds)')

        # Additional wait for AMCL to process
        self.get_logger().info('Giving AMCL 2 more seconds to process initial pose...')
        time.sleep(2.0)
    ...
    def proceed_to_next_waypoint(self) -> None:
        ...
        # Pause at current waypoint using non-blocking timer
    ...
    def start_navigation(self) -> None:
        ...
        # Create a timer to periodically check for AMCL convergence
    ...
    def check_amcl_convergence(self) -> None:
        ...
        if self.amcl_pose_received:
            # AMCL has converged successfully
            ...
            # Wait for particle filter to stabilize using timer
            ...
            # Timeout reached without AMCL convergence
            if self.amcl_check_timer is not None:
                self.amcl_check_timer.cancel()
                self.amcl_check_timer = None
            ...
            # Wait a bit longer before attempting navigation anyway
        ...
        elif int(elapsed) % 2 == 0 and int(elapsed * 10) % 10 == 0:
            # Log every 2 seconds
    ...
    def start_first_waypoint(self) -> None:
        ...
        # Cancel the one-shot stabilization timer
...
def main(args=None):
    ...
    try:
        ...
        # This ensures the executor is running and can process callbacks
